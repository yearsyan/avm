...
public:
    ...
    virtual ReportAction requestConsent(const CrashReportInfo& report) = 0;
    virtual void reportCompleted(const CrashReportInfo& report) {
        printf("Report %s is available remotely as: %s.\n",
               report.uuid.c_str(), report.id.c_str());
    }
};
